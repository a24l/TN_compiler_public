...
def generate_M(
    ...
    """
    Generate a single Haar-random target unitary M ∈ U(d^n).

    M acts on the FULL n-qubit Hilbert space — it is the quantum
    operation you want to learn.  Its size scales as:

        size = d^n  →  2^n for qubits (d=2)

    ┌──────────┬──────────┬──────────────────────────────────────┐
    │ n_qubits │  size    │  memory (complex128)                 │
    ├──────────┼──────────┼──────────────────────────────────────┤
    │    1     │   2×2    │  ~0.00 MB                            │
    │    2     │   4×4    │  ~0.00 MB                            │
    │    3     │   8×8    │  ~0.00 MB                            │
    │    4     │  16×16   │  ~0.00 MB   ← NFL paper default      │
    │    5     │  32×32   │  ~0.01 MB                            │
    │    6     │  64×64   │  ~0.06 MB                            │
    │    8     │ 256×256  │  ~1.05 MB                            │
    │   10     │1024×1024 │ ~16.78 MB                            │
    │   12     │4096×4096 │ ~268.4 MB  ← memory warning          │
    └──────────┴──────────┴──────────────────────────────────────┘

    Parameters
    ----------
    n_qubits : number of qubits.  Controls the size of M.
    d        : physical dimension per qubit.  d=2 for qubits (default).
    seed     : RNG seed for reproducibility.
    verbose  : print summary.

    Returns
    -------
    M : np.ndarray  shape (d^n, d^n)  complex128
        Satisfies M†M = I  to machine precision.
    """
    rng  = np.random.default_rng(seed)
    size = d ** n_qubits

    # Memory warning for large M
    mem_mb = size * size * 16 / 1e6
    if mem_mb > 200:
        print(f"  ⚠  WARNING: M will be {size}×{size} = {mem_mb:.1f} MB. "
              f"Consider reducing n_qubits.")

    if verbose:
        print("=" * 58)
        print("  Generating Target Unitary M")
        print("=" * 58)
        print(f"  n_qubits             : {n_qubits}")
        print(f"  d  (physical dim)    : {d}  {'← qubits' if d==2 else ''}")
        print(f"  M size               : {size} × {size}  "
              f"(d^n = {d}^{n_qubits})")
        print(f"  M ∈ group            : U({size})  "
              f"(full {n_qubits}-qubit unitary group)")
        print(f"  Memory               : {mem_mb:.4f} MB  (complex128)")
        print(f"  Seed                 : {seed}")
        print("-" * 58)

    M = haar_unitary(size, rng)

    ok = verify_unitary(M)
    if verbose:
        residual = np.max(np.abs(M.conj().T @ M - np.eye(size)))
        print(f"  Unitarity check      : {'✓ passed' if ok else '✗ FAILED'}")
        print(f"  max ‖M†M - I‖        : {residual:.2e}")
        print(f"  Determinant |det(M)| : {abs(np.linalg.det(M)):.8f}  "
              f"(should be 1.0)")
        print("=" * 58)
...
def build_dataset(
    ...
    Phi = (M @ X.T).T   # now ‖Phi_j‖ = 1 guaranteed since M is unitary
...
if __name__ == "__main__":
    ...
    POOL_PATH  = "output/pool_mps1_D2.npz"
    SAVE_PATH  = "output/dataset_M4_D2_t6000"  # output path (without .npz)
    # ── END PARAMETERS ─────────────────────────────────────────────

    # Step 1 — generate M
    M = generate_M(n_qubits=N_QUBITS, d=D_PHYS, seed=M_SEED, verbose=True)
